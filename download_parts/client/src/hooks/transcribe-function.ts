// Nova função para transcriber áudios
export async function transcribeAudioSegments(
  segments: Array<{ id: string; blob: Blob; duration: number }>,
  ...
  try {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      console.log(`Processando segmento ${i+1}/${segments.length}: ${segment.id} (tamanho: ${segment.blob.size} bytes)`);
    ...
    console.log(`Transcrição completa obtida (${allTranscriptions.length}/${segments.length} segmentos): ${combinedTranscription.substring(0, 100)}...`);
